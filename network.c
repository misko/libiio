...
static ssize_t network_get_buffer_metadata_status(const struct iio_device *dev,
...
static ssize_t network_drain_buffer_metadata(const struct iio_device *dev,
		void *metadata, size_t metadata_capacity)
{
	...
	if (!capability || strcmp(capability, "1"))
		return -ENOSYS;
	iio_mutex_lock(pdata->lock);
	if (pdata->io_ctx.fd < 0 || pdata->io_ctx.cancelled) {
		ret = -EBADF;
	} else {
		ret = iiod_client_drain_buffer_metadata_unlocked(
			ctx_pdata->iiod_client, &pdata->io_ctx, dev, metadata,
			metadata_capacity, &stream_valid);
		/* Do not let a partial blob be interpreted as a later command reply. */
		if (!stream_valid)
			network_cancel(dev);
	}
	iio_mutex_unlock(pdata->lock);
...
static int network_cancel_buffer_metadata_session(const struct iio_device *dev)
{
	struct iio_context_pdata *ctx_pdata = iio_context_get_pdata(dev->ctx);
	struct iio_device_pdata *pdata = dev->pdata;
	const char *capability = iio_context_get_attr_value(dev->ctx,
	...
	if (pdata->io_ctx.fd < 0 || pdata->io_ctx.cancelled)
		...
		ret = iiod_client_cancel_buffer_metadata_unlocked(
...
static int network_prequeue_metadata_reads_async_policy(
...
static const struct iio_backend_ops network_ops = {
	...
	.prequeue_metadata_reads_async_policy =
		network_prequeue_metadata_reads_async_policy,
	.write = network_write,
#ifdef WITH_NETWORK_GET_BUFFER
