		      ...
		      int ep0_fd, long cpu_affinity, long rw_cpu_affinity)
{
	...
	ret = iio_context_add_attr(ctx, "iio,buffer-metadata-abi-versions",
		"1,2,3,4");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	...
	ret = iio_context_add_attr(ctx, "iio,buffer-metadata-record", "7");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	ret = iio_context_add_attr(ctx, "iio,buffer-metadata-features",
	...
	ret = iio_context_add_attr(ctx, "iio,buffer-ddr-ring", "1");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	ret = iio_context_add_attr(ctx, "iio,buffer-ddr-ring-max-iq-bytes",
		"200000000");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	ret = iio_context_add_attr(ctx, "iio,buffer-ddr-ring-modes",
		"finite,continuous");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	ret = iio_context_add_attr(ctx, "iio,buffer-metadata-status", "1");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
	}
	ret = iio_context_add_attr(ctx, "iio,buffer-metadata-status-versions",
		"1,2");
	if (ret < 0) {
		iio_context_destroy(ctx);
		return EXIT_FAILURE;
